...
use crate::init::context::{
    CTX_KEY_PASSPHRASE_INIT, CTX_KEY_ROOT_PASSPHRASE_BOOT, CTX_KEY_VAR_PASSPHRASE_BOOT, InitContext,
};
    ...
    if ctx.is_first_boot() {
        if let Some(passphrase) = ctx.get_bin(CTX_KEY_PASSPHRASE_INIT).cloned() {
            ...
            return Err(config_err(
                format!(
                    "this oneshot requires mounting /var and no '{CLOUD_INIT_FILE_INIT_PW}' file was found in the cloud-init"
                ),
                None,
            ));
        }
...
fn mount_var(ctx: &mut InitContext, _oneshot_ctx: &OneShotContext) -> Result<()> {
    if !ctx.is_first_boot() {
        ...
        return Err(validation_err(
            "this oneshot requires mounting /var however this is only supported before init has occurred",
            None,
        ));
    }
