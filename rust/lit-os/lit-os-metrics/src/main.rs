...
fn handle_metrics(args: CmdArgs, os_query: OSQuery) -> Result<()> {
    for query in &args.query {
        match query {
            Query::RunningProcess => {
                add_info_metrics::<RunningProcess>(&os_query, running_process())?
            }
            Query::EstablishedOutbound => {
                add_info_metrics::<EstablishedOutbound>(&os_query, established_outbound())?
            }
            Query::CronJob => add_info_metrics::<CronJob>(&os_query, crontab())?,
            Query::LoginHistory => add_info_metrics::<LoginHistory>(&os_query, login_history())?,
            Query::OsInfo => add_info_metrics::<OsInfo>(&os_query, os_info())?,
            Query::InterfaceAddress => {
                add_info_metrics::<InterfaceAddress>(&os_query, interface_addresses())?
            }
            Query::DockerRunningContainers => {
                add_info_metrics::<DockerRunningContainers>(&os_query, docker_running_containers())?
            }
            Query::DebianPackage => {
                add_info_metrics::<DebianPackage>(&os_query, debian_packages())?
            }
            ...
            Query::ListeningPorts => {
                add_info_metrics::<ListeningPort>(&os_query, listening_ports())?
            }
            Query::KernelInfo => add_info_metrics::<KernelInfo>(&os_query, kernel_info())?,
            Query::Uptime => add_gauge_metrics::<Uptime>(&os_query, uptime())?,
            Query::Iptables => add_info_metrics::<IptablesRule>(&os_query, iptables())?,
            Query::SystemInfo => add_info_metrics::<SystemInfo>(&os_query, system_info())?,
        }
    }
