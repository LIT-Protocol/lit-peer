use crate::config::{CFG_KEY_LITOS_HOST_RESERVED_MEM, LitCliOsConfig};
use crate::guest::instance::{GuestInstanceItem, active_guest_instance_stats};
...
use lit_os_core::error::{blockchain_err, parser_err, unexpected_err, validation_err};
...
impl GuestInstanceItemHelper for GuestInstanceItem {
    ...
    async fn is_active_on_network(&self, cfg: &LitConfig) -> Result<bool> {
        ...
        let realm_id =
            staking_contract.get_realm_id_for_staker_address(staker_address).call().await.map_err(
...
pub fn can_enable_instance<S: AsRef<str>>(cfg: &LitConfig, instance_mem: S) -> Result<()> {
    ...
    let avail = total.saturating_sub(cur_bytes) as f64;
