...
use terminal_spinners::{DOTS, SpinnerBuilder, SpinnerHandle};

use crate::config::LitCliOsConfig;
use crate::error::{Result, generic_err};
use crate::guest::common::{MANIFEST_FILE, MANIFEST_PROOF_FILE};
use crate::guest::template::{GuestTemplateItem, find_one_guest_template};
use lit_attestation::attestation::{
    DATA_KEY_MANIFEST_HASH, DATA_KEY_UNIX_TIME, FromSystem, TryGenerate,
};
...
use lit_os_prov_core::release::common::manifest::{
    ASSET_KEY_AMD_OVMF, ASSET_KEY_BUILD_PEM, ASSET_KEY_GUEST_INITRD, ASSET_KEY_GUEST_KERNEL,
    ASSET_KEY_GUEST_KERNEL_CMDLINE, ASSET_KEY_RELEASE_ENV, RELEASE_FILE_INITIAL_PASSWORD,
    load_release_manifest_file,
};
use lit_os_prov_core::release::create::types::{CreateRelease, CreateReleaseRequest};

use crate::cmd::os::guest::template::release_publish::generate_and_publish_release_info;
...
/// Valid network names for releases.
///
/// This enum ensures type-safe validation at CLI parse time - invalid network names
/// will be rejected before any release operations begin, preventing misconfiguration.
#[derive(Debug, Clone, ValueEnum)]
pub(crate) enum NetworkName {
    #[value(name = "datil-prod")]
    DatilProd,
    #[value(name = "datil-test")]
    DatilTest,
    #[value(name = "datil-dev")]
    DatilDev,
    #[value(name = "datil-proto")]
    DatilProto,
...
pub(crate) struct GuestTemplateRelease {
    ...
    /// Target network name for GitHub release publication.
    /// If not provided, GitHub publishing will be skipped (IPFS release still happens).
    /// Valid values: internal-dev, datil-prod, datil-test, datil-dev, naga-prod, etc.
    #[arg(long, value_name = "NETWORK")]
    pub(crate) network_name: Option<NetworkName>,
...
pub(crate) async fn handle_cmd_os_guest_template_release(
    ...
    if let Some(item) = find_one_guest_template(&cfg, None, None, Some(&args.id), None) {
        ...
        let release = do_release_template(&cfg, &opts, &args, &item, admin_key).await;

        let release_id = release.release_id().clone();

        // Check if there is a docker image to publish.
        if let Some(img) = &item.build_env.build_env_img {
            if let Err(e) =
                do_publish_image(img, item.build_env.build_id.as_ref().unwrap(), &release_id).await
            {
                eprintln!("Error publishing Docker image: {}", e);
                return false;
            }
        }
    }
...
/// Publishes the build environment image to Docker Hub.
///
/// 'do_publish_image' performs three main tasks:
/// 1. Retags the local Docker image. The original tag is based on the `BUILD_ID`,
///    and it is retagged with the `release_id`. This is done to create a unique,
///    traceable association between the Docker image and a specific release on a
///    specific subnet.
/// 2. Logs into Docker Hub. It first checks for an existing session and, if not found,
...
async fn do_publish_image(image_repo: &str, build_id: &str, release_id: &str) -> Result<()> {
    ...
    let local_image_name = format!("{}:{}", image_repo, build_id);
    let remote_image_name = format!("{}:{}", image_repo, release_id);

    // Tag the image.
    println!("Tagging image: {} as {}", local_image_name, remote_image_name);
    let tag_output = Command::new("docker")
        .arg("tag")
        .arg(&local_image_name)
        .arg(&remote_image_name)
        .output()
        .map_err(|e| generic_err(e, Some("failed to execute docker tag".into())))?;

    if !tag_output.status.success() {
        let stderr = String::from_utf8_lossy(&tag_output.stderr);
        return Err(generic_err(
            format!(
                "failed to tag docker image (exit code: {:?}): {}",
                tag_output.status.code(),
                stderr.trim()
            ),
            None,
        ));
    ...
    // Check for existing login by inspecting the docker config file.
    let mut logged_in = false;
    if let Ok(home_dir) = env::var("HOME") {
        let config_path = Path::new(&home_dir).join(".docker/config.json");
        if config_path.exists() {
            if let Ok(contents) = fs::read_to_string(config_path) {
                if contents.contains("\"auths\":") {
                    logged_in = true;
                }
            }
        }
    ...
    // If not logged in, try to log in using env vars.
    if !logged_in {
        println!("Not logged into Docker Hub. Attempting login with environment variables...");

        let username = env::var("DOCKER_HUB_USERNAME").map_err(|_| {
            generic_err(
                "DOCKER_HUB_USERNAME environment variable must be set for automated login.",
                None,
            )
        })?;
        let password = env::var("DOCKER_HUB_PASSWORD").map_err(|_| {
            generic_err(
                "DOCKER_HUB_PASSWORD environment variable must be set for automated login.",
                None,
            )
        })?;
        ...
        let mut child = Command::new("docker")
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| generic_err(e, Some("failed to spawn docker login".into())))?;

        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(password.as_bytes()).map_err(|e| {
                generic_err(e, Some("failed to write password to docker login stdin".into()))
            })?;
        }

        let output = child
            .wait_with_output()
            .map_err(|e| generic_err(e, Some("failed to wait for docker login".into())))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(generic_err(
                format!(
                    "failed to login to docker (exit code: {:?}): {}",
                    output.status.code(),
                    stderr.trim()
                ),
                None,
    ...
    // Push the image.
    println!("Pushing image: {}", remote_image_name);
    let push_output = Command::new("docker")
        .arg("push")
        .arg(&remote_image_name)
        .output()
        .map_err(|e| generic_err(e, Some("failed to execute docker push".into())))?;

    if !push_output.status.success() {
        let stderr = String::from_utf8_lossy(&push_output.stderr);
        return Err(generic_err(
            format!(
                "failed to push docker image (exit code: {:?}): {}",
    ...
    println!("✅ Successfully pushed image: {}", Green.paint(&remote_image_name));
    Ok(())
}
...
pub(crate) async fn do_release_template(
    ...
    let subnet_id = remove_0x_prefix(args.subnet_id.clone().unwrap_or_else(|| {
    ...
    let request = CreateRelease::new(release_id, manifest_cid.clone(), password_bytes, public_key_bytes);

    if !args.push_only {
        submit_release(cfg, request.clone(), &subnet_id, admin_key, build_release_env, opts.quiet()).await;
    }

    // After the primary release artifacts are created, publish the release info to GitHub.
    // This is optional and only happens if --network-name is provided.
    // This is a best-effort, non-critical step; a failure here should not block the release.
    if args.network_name.is_some() {
        let manifest_cid_clone = manifest_cid.clone();
        if let Err(e) =
            generate_and_publish_release_info(cfg, args, item, &request, &manifest_cid_clone).await
        {
            eprintln!("Warning: Failed to publish release information to GitHub: {}", e);
        }
    } else {
        println!("ℹ️  Skipping GitHub release publication (no --network-name provided)");
    }
...
async fn generate_manifest_with_proof(
    ...
    let vcpu_type = <GuestCpuType as FromStr>::from_str(vcpu_type).unwrap();
...
async fn request_remote_pinning(cfg: &LitConfig, pins: &mut [IpfsPinItem], quiet: bool) {
