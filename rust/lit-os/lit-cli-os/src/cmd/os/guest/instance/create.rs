...
use crate::guest::instance::helper::{GuestInstanceHelper, can_enable_instance};
use crate::guest::instance::oneshot::create_oneshot_actions;
use crate::guest::instance::release::{IssuedRelease, download_and_install_release};
use crate::guest::instance::{GuestInstanceItem, find_one_guest_instance};
use crate::guest::template::{GuestTemplateItem, find_guest_templates};
...
fn create_lit_custom_cfg<S: AsRef<str> + Into<String>>(
    ...
    if let Some(from) = from_item {
        ...
        if GuestType::Custom.eq(&instance_type) {
            ...
            let wallet_key = match cfg.wallet_key(&wallet_key_cfg_key).ok() {
                Some(key) => key,
                None => {
                    eprintln!(
            ...
            if GuestType::Node.eq(&instance_type) {
                ...
                let node_staker_address = match node_args.node_staker_address.clone() {
                ...
                let node_admin_address = match node_args.node_admin_address.clone() {
                    Some(address) => address,
                    None => {
                        eprintln!(
                ...
                let coms_keys_sender_privkey =
                    cfg.coms_sender_key().expect("coms sender private key is required");
                let coms_keys_receiver_privkey =
                    cfg.coms_receiver_key().expect("coms receiver private key is required");
