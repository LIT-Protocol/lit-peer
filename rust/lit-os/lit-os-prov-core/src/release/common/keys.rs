...
use sev_snp_utilities::{LaunchDigest, fingerprint_id_key};
use tracing::info;
...
use lit_os_core::utils::sev_snp::{DEFAULT_SEV_SNP_POLICY, sev_snp_host_identity};

use crate::config::LitOsProvConfig;
use crate::error::{
    EC, Result, conversion_err, io_err, unexpected_err, validation_err, validation_err_code,
};
...
pub fn issue_host_identity(
    ...
    info!(
        ...
        vcpu_type = ?vcpus_type,
        vcpus = vcpus,
        image_id = image_id.hex(),
        family_id = family_id.hex(),
        family_id_fp = bytes_to_hex(fingerprint),
        family_id_seed = bytes_to_hex(seed),
