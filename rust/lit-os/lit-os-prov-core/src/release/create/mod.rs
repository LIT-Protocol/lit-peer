...
use crate::error::{
    EC, Result, blockchain_err, blockchain_err_code, io_err_code, unexpected_err_code,
    validation_err, validation_err_code,
};
use crate::release::common::keys::{
    ID_KEY_FILE_NAME, create_id_pem_key, extract_host_identity_fingerprint,
};
use crate::release::common::manifest::{
    RELEASE_FILE_INITIAL_PASSWORD, ReleaseManifest, load_release_manifest,
};
...
pub async fn create_release(
    ...
    info!(release_id = req.release_id(), "Creating release");
    ...
    info!(release_id = req.release_id(),
        env = ?build_env.env()?,
        guest_type = ?build_env.guest_type()?,
        status = ?ReleaseStatus::Active,
        platform = ?build_env.guest_cpu_type()?,
...
async fn verify_release_not_exist(
    contract: &ReleaseRegister<SignerMiddleware<Arc<Provider<Http>>, Wallet<SigningKey>>>,
    req: &CreateRelease,
) -> Result<()> {
...
fn create_release_id_pem_key(
    ...
    info!(release_id = release_id, "Creating release id key: {}", ID_KEY_FILE_NAME);
