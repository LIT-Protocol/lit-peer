...
use lit_core::config::{ENV_LIT_CONFIG_FILE, LitConfig};
...
async fn test_host_commands_listener() -> Result<(), Box<dyn std::error::Error>> {
    let log_guard = init_file_logging(); // initialize tracing subscriber as HostCommandsListener uses the tracing crate
    ...
    let log_file_path = current_path.join("tests").join("test.log");
    ...
    // Restart event
    assert!(
        log_lines.iter().any(|line| line.contains("New event in event listener: RestartFilter"))
    );
    assert!(log_lines.iter().any(|line| line.contains("Restart event")));
...
fn init_file_logging() -> WorkerGuard {
    ...
    let file =
        OpenOptions::new().create(true).write(true).truncate(true).open("tests/test.log").unwrap();
