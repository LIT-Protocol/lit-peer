...
#[cfg(feature = "upgrade")]
use crate::events::Upgrade;
use crate::events::{EventHandler, Restart};
...
impl HostCommandsListener {
    ...
    pub async fn listen_for_events(&self) -> Result<()> {
        ...
        debug!(
            "HostCommandsListener listening for host command events on {:?} starting from block {:?}",
            self.host_commands_contract, current_block
        );
        loop {
            tokio::select! {
                // watchdog time
                        ...
                        match event {
                            Ok(log) => {
                                ...
                                match log.clone() {
                                    ...
                                    HostCommandsEvents::UpgradeFilter(_) => {
                                        debug!("Upgrade feature is disabled at build time; ignoring Upgrade event");
                                    }
                                    _ => {}
                                }
                            }
