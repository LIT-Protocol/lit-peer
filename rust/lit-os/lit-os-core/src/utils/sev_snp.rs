...
use sev_snp_utilities::{
    BlockSigner, FamilyId, IdBlock, ImageId, LaunchDigest, SevMode, ToBase64, calc_launch_digest,
};

use lit_core::utils::binary::bytes_to_hex;
use lit_core::utils::hash::sha512_file;

use crate::error::{Error, Result, io_err, sev_snp_err, validation_err};
...
pub(crate) fn maybe_sha512_file_to_hex(path: &Path) -> String {
    if let Ok(hash) = sha512_file(path) { bytes_to_hex(hash) } else { "None".to_string() }
}
