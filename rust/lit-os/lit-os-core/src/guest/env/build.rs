...
use lit_core::utils::env::{Error as EnvError, parse_env_to_map};
use lit_core::utils::option::bool_option_to_bool;

use crate::error::{Result, config_err, conversion_err, generic_err};
...
pub struct GuestBuildEnv {
    ...
    pub build_env_img: Option<String>,
    pub build_bin_hash: Option<String>,
...
pub fn load_guest_build_env<R: Read>(
    ...
    for (key, value) in env_data {
        match key {
            ...
            k if k == apply_prefix(prefix, "BUILD_ENV_IMG") => env.build_env_img = Some(value),
            k if k == apply_prefix(prefix, "BUILD_BIN_HASH") => env.build_bin_hash = Some(value),
