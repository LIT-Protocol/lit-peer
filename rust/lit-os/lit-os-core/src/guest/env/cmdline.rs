...
use crate::error::{Result, config_err, conversion_err, generic_err, io_err};
        ...
        if expect_release {
            ...
            if expect_subnet {
                ...
                if !release_id.starts_with(&expected_prefix) {
                    return Err(generic_err(
                        format!(
                            "GuestCmdLine release_id does not match '<build_id><subnet_id>' prefix: '{release_id}' vs '{expected_prefix:?}'"
                        ),
                        None,
                    ));
                }
