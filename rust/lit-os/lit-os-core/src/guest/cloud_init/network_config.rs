...
use crate::error::{Result, config_err, generic_err, io_err, serializer_err, validation_err};
        ...
        match iface_kind {
            ...
            IfaceKind::External => {
                if !is_optional && (!has_ipv4 || has_ipv6) {
                    return Err(validation_err(
                        format!(
                            "invalid config, external iface '{iface}' not marked as optional with no config"
                        ),
                        None,
                    ));
                }
