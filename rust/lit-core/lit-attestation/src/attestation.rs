...
use crate::error::{
    EC, Result, attestation_err, attestation_err_code, conversion_err, sev_snp_err_code,
    validation_err, validation_err_code,
};
#[cfg(feature = "generate")]
use crate::error::{config_err, generic_err};
#[cfg(feature = "generate-via-service")]
use crate::service::client::{ATTESTATION_SERVICE_CLIENT, AttestationServiceClient};
#[cfg(feature = "generate-via-service")]
...
pub trait FromSystem {
    ...
    fn from_system() -> Option<Self::Output>;
}

impl FromSystem for AttestationType {
    type Output = Self;

    fn from_system() -> Option<AttestationType> {
...
impl Attestation {
    ...
    /// Updates hasher with Attestation data and signatures up to given signature index.
    ///
    /// For signing: pass `None` for last_sig_idx to include all existing signatures.
    /// For verification: pass specific index to recreate the historical state when
...
fn last_sig_idx_before(idx: usize) -> Option<isize> {
    if idx > 0 { Some(idx as isize - 1) } else { Some(-1) }
}
...
impl AmdSevSnpAttestation for Attestation {
    ...
    fn generate(mut self) -> Result<Self> {
        ...
        let report =
            AttestationReport::request_raw(report_data.as_slice()).map_err(|e| match e.kind() {
                sev_snp_utilities::error::Kind::Fetch => sev_snp_err_code(
                    e,
                    EC::AttestationRequestFailed,
                    Some("failed to fetch AMD SEV-SNP attestation report".into()),
                ),
                sev_snp_utilities::error::Kind::LockTimeout => sev_snp_err_code(
                    e,
                    EC::AttestationRequestFailed,
                    Some("failed to obtain lock for AMD SEV-SNP attestation report request".into()),
                ),
                _ => sev_snp_err_code(
                    e,
...
mod tests {
    ...
    async fn sha512_test() {
        ...
        assert_eq!(
            bytes_to_hex(&attestation.sha512(None).to_vec()),
            "978ace8d4b976fefbe3041b5c21c25e9ac67a4870264a4f0924d20f432df220751f01295274bf483a18e56a18ca6439a4ae28b74023860232841aa04c3c679b1"
        );
        ...
        assert_eq!(
            bytes_to_hex(&attestation.sha512(None).to_vec()),
            "a1868d24188f029b0cce4f337b7cc1c57ebf7173221c2b2d5162272ca7547d85648c00a5442bf3f66b43acc732b5c0529cf455bbf63b3c757208645d16b27f62"
        );
        ...
        assert_eq!(
            bytes_to_hex(&attestation.sha512(None).to_vec()),
            "4abbe406103a9e8fa428eb3fe457adcdf6375b257cdec5d4b064f3f4c2edb14b38af8b82a8b103b945e62d9eb807065e4b4dd7d95f36ac65989add41dbf85b49"
        );
    }
    ...
    async fn sha512_truncated_32_test() {
        ...
        assert_eq!(
            bytes_to_hex(&trunc),
            "978ace8d4b976fefbe3041b5c21c25e9ac67a4870264a4f0924d20f432df2207"
        );
    }
    ...
    async fn admin_signer_sign_test() {
        ...
        assert_eq!(
            ...
            "304402205cedbfa246aa4bc89248baabe08375a35cf2f740dcabc2b96e3d5601198104020220780473d632cf4566b494ebe028207e2172513788c2f5356b85c0dcded057068b"
        );
    ...
    async fn attestation_to_json_test() {
        ...
        assert_eq!(
            json,
            "{\"type\":\"ADMIN_SIGNED\",\"noonce\":\"ZmRzZGZkc2YyMzEyMzEyMzMyMw==\",\"data\":{\"one\":\"MQ==\",\"three\":\"Mw==\",\"two\":\"Mg==\"},\"signatures\":[\"MEQCIFztv6JGqkvIkki6q+CDdaNc8vdA3KvCuW49VgEZgQQCAiB4BHPWMs9FZrSU6+AoIH4hclE3iML1NWuFwNze0FcGiwAEHowB1+oktqkaS6oKvKw2nOyCWIgZdUF5IvUks4/mUT85Bpy3YlSEaQKlzOyZnKf5L+sAzVeuS4e/YL0UEgBtXQ==\"],\"report\":null}"
        );
...
#[tokio::test]
#[ignore = "This test must be run as superuser from within a SEV-SNP guest VM"]
#[cfg(feature = "generate-via-system")]
async fn amdsev_snp_verify_success_test() {
    ...
    if !Path::new("/dev/sev-guest").exists() {
        panic!("/dev/sev-guest not found, run in snp guest VM");
    }
    let noonce: Vec<u8> = "fdsdfdsf23123123323".into();
    let mut attestation = Attestation::new(AttestationType::AmdSevSnp, noonce.clone())
        .await
        .expect("failed to create Attestation");
    ...
    static DUMMY_PRIVATE_KEY: &str =
        "5bbd72a9de74d335f1a05ee2fc6f59ff9ab899c497541a015d556990513affff";
    attestation.sign(DUMMY_PRIVATE_KEY).expect("failed to sign attestation"); // Now sign the attestation

    let complete_attestation =
        attestation.generate_via_system().await.expect("failed to generate attestation");
    // the attestation is now finalized
    assert!(complete_attestation.verify().await.is_ok());
}
