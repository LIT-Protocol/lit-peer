...
use crate::error::{Result, blockchain_err, config_err};
use crate::verification::cache::{CACHE, CacheValue};
...
pub(crate) async fn staking_has_allowed_node_signing_public_key(
    contract: &Staking<Provider<Http>>, subnet_id: &str, public_key: &[u8], label: &str,
) -> Result<bool> {
    ...
    let staking_address =
        contract.node_address_to_staker_address_across_realms(node_address).call().await.map_err(
            |e| {
                blockchain_err(
                    e,
    ...
    if staking_address.is_zero() {
        debug!(
            ...
            node_address = node_address.encode_hex(),
            staking_address = staking_address.encode_hex(),
    ...
    let validator =
        contract.validator_by_staker_address(staking_address).call().await.map_err(|e| {
    ...
    let validator_balance = contract.balance_of(staking_address).call().await.map_err(|e| {
    ...
    if !valid {
        debug!(
            ...
            node_address = node_address.encode_hex(),
            staking_address = staking_address.encode_hex(),
            validator_valid = validator_ip != 0,
