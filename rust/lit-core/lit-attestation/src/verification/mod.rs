...
use lit_blockchain::util::release::{RELEASE_ID_STR_LEN, subnet_id_from_release_id};
...
use crate::attestation::{AdminSignedType, Attestation, DATA_KEY_RELEASE_ID};
use crate::error::{
    EC, Result, attestation_err_code, conversion_err, conversion_err_code, unexpected_err_code,
    validation_err_code,
};
use crate::utils::sev_snp::FamilyIdBuilder;
use crate::verification::blockchain::{
    get_release, get_release_register_contract, get_staking_contract,
    release_has_allowed_admin_signing_public_key, release_has_allowed_author_key_digest,
    staking_has_allowed_node_signing_public_key, unwrap_or_create_contract_resolver,
};
...
pub(crate) async fn verify_admin_signed(
    ...
    match verify_signatures(cfg, resolver, data, policy, allowed, None, "verify_admin_signed").await
    {
        Ok((passed_type, public_key)) => {
            trace!(
                data_type = ?data.typ(),
                auth_type = ?passed_type,
                auth_public_key = bytes_to_hex(public_key.serialize()),
...
pub(crate) async fn verify_release(
    ...
    match data.typ() {
        AttestationType::AmdSevSnp => {
            ...
            trace!(
                data_type = data.typ().to_string(),
                auth_type = auth_type.map(|v| v.to_string()).unwrap_or_default(),
                auth_public_key =
                    auth_public_key.map(|v| bytes_to_hex(v.serialize())).unwrap_or_default(),
                build_public_key = build_public_key_vec.map(bytes_to_hex).unwrap_or_default(),
                release_id = &release_id,
                image_id = report.image_id.hex(),
                family_id = report.family_id.hex(),
                family_id_fp = bytes_to_hex(family_id.fingerprint),
                family_id_seed = bytes_to_hex(family_id.seed),
                "Attestation verify OK"
            );
        }
        typ => {
