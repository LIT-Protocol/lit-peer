...
use crate::error::{
    EC, Result, attestation_err, attestation_err_code, conversion_err, sev_snp_err,
    sev_snp_err_code, validation_err, validation_err_code,
};
...
impl Kdf {
    ...
    pub async fn try_derive<S: AsRef<str>>(
        cfg: &LitConfig, context: S, namespace: Option<String>,
    ) -> Result<[u8; 32]> {
        ...
        Self::try_new(cfg, kdf_type, namespace)?.derive(context.as_ref()).await
    }
