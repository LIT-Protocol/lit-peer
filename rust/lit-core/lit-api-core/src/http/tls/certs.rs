...
use scc::HashIndex;
use zerossl::certs::csr::{Csr, generate_ca, generate_ca_signed_cert, generate_rsa_2048_priv_key};
...
use crate::config::{
    CFG_KEY_DOMAIN, CFG_KEY_ENABLED, CFG_KEY_TLS_AUTO_CHECK_INITIAL_SEC,
    ...
    CFG_SECTION_HTTPS, LitApiConfig,
};
use crate::error;
use crate::error::Result;
use crate::http::rocket::event::{Event, EventDataKey, EventManager};

pub static CERT_PKI_VALIDATIONS: LazyLock<HashIndex<String, Vec<u8>>> =
...
pub(crate) fn ep_certs_pki_validation(id: &str) -> Option<Vec<u8>> {
    ...
    if let Some(content) = CERT_PKI_VALIDATIONS.get_sync(&id) {
...
pub struct CertManager {
    ...
    quit_mu: Arc<AtomicBool>,
}

impl CertManager {
    pub fn new(cfg: ReloadableLitConfig) -> Self {
        Self { cfg, thread_join_handle: None, quit_mu: Arc::new(AtomicBool::new(false)) }
    }
    ...
    pub fn start(&mut self, event_manager: EventManager) {
        ...
        self.thread_join_handle = Some(thread::spawn(move || {
            ...
            let mut quit = quit_mu.load(Ordering::Acquire);
            while !quit {
...
async fn check_certs(
    cfg: Arc<LitConfig>, quit_mu: Arc<AtomicBool>, event_manager: &EventManager,
    common_name: &String,
) -> Result<()> {
    let threshold_days =
...
async fn create_or_renew(
    cfg: &LitConfig, quit_mu: Arc<AtomicBool>, event_manager: &EventManager, common_name: &String,
) -> Result<()> {
    ...
    match zerossl.purge_certificates(common_name.clone(), purge_pending, purge_active).await {
        Ok(_) => {}
        Err(e) => {
            warn!("Non-fatal: error purging certificates for '{}': {:?}", common_name, e);
        }
    }
...
async fn store_certs_and_trigger_event(
    ...
    // Send event
    let mut event_data: HashMap<EventDataKey, Vec<u8>> = HashMap::with_capacity(4);
...
async fn store_validation_and_trigger_event(
    ...
    // Construct content
    let content = validation_contents.join("\n");

    // Store ID
    CERT_PKI_VALIDATIONS
        .insert_sync(validation_id.clone(), content.clone().into_bytes())
        .map_err(|_e| error::certs_err("Unable insert validation content", None))?;
...
async fn verify_cert(
    cfg: &LitConfig, quit_mu: Arc<AtomicBool>, zerossl: &Client, cert_id: String,
) -> Result<()> {
    let mut quit = quit_mu.load(Ordering::Acquire);
...
fn set_bool_mu(the_mu: Arc<AtomicBool>, new_val: bool) {
    the_mu.store(new_val, Ordering::Release);
}

fn interruptable_sleep(sleep_ms: u64, quit_mu: Arc<AtomicBool>) {
    let mut quit;
...
async fn async_interruptable_sleep(sleep_ms: u64, quit_mu: Arc<AtomicBool>) {
    let mut quit;
    for _ in 0..(sleep_ms / INTERRUPTABLE_SLEEP_MS) {
        quit = quit_mu.load(Ordering::Acquire);
        if !quit {
