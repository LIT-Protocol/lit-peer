...
use crate::contracts::{
    ALLOWLIST_CONTRACT, BACKUP_RECOVERY_CONTRACT, CONTRACT_RESOLVER_CONTRACT, FORWARDER_CONTRACT,
    HOST_COMMANDS_CONTRACT, LEDGER_CONTRACT, LIT_TOKEN_CONTRACT, MULTI_SENDER_CONTRACT,
    PAYMENT_DELEGATION_CONTRACT, PKP_HELPER_CONTRACT, PKP_NFT_CONTRACT, PKP_NFT_METADATA_CONTRACT,
    PKP_PERMISSIONS_CONTRACT, PRICE_FEED_CONTRACT, PUB_KEY_ROUTER_CONTRACT,
    RELEASE_REGISTER_CONTRACT, STAKING_CONTRACT,
};
use crate::error::{Result, blockchain_err, conversion_err, lock_err};
...
impl ContractResolver {
            ...
            if let Some(val) = val.contracts().get(contract_key.as_ref()) {
                if !val.address().is_empty() {
                    match val.address().parse::<Address>() {
                        ...
                        Err(err) => {
                            warn!(
                                "failed to parse address '{}' found in subnet ('{}') resolver config for contract key: {} - {:?}",
                                val.address(),
                                &self.subnet_id,
                                contract_key.as_ref(),
                                err
    ...
    pub async fn staking_contract_with_gas_relay(
        ...
        Staking::load_with_gas_relay(
            cfg,
            *self.resolve(cfg, STAKING_CONTRACT).await?.address(),
            *self.resolve(cfg, FORWARDER_CONTRACT).await?.address(),
            self.wallet_key_as_str(),
            meta_signer_key,
        )
    }
    ...
    // Ledger
    pub async fn ledger_contract(&self, cfg: &LitConfig) -> Result<Ledger<Provider<Http>>> {
        Ledger::load(cfg, *self.resolve(cfg, LEDGER_CONTRACT).await?.address())
    }
    ...
    // PriceFeed
    pub async fn price_feed_contract(&self, cfg: &LitConfig) -> Result<PriceFeed<Provider<Http>>> {
        PriceFeed::load(cfg, *self.resolve(cfg, PRICE_FEED_CONTRACT).await?.address())
    }
        ...
        if let Some(address) = call.tx.to_addr() {
            if let Some(contract_key) = self.contract_key_by_address(address)? {
                ...
                warn!(
                    ...
                    "ContractResolver::prepare(): unable to resolve contract_key for addr '{}' (should be in cache)",
                    address
                );
            }
        } else {
...
impl SubnetResolverCacheEntry {
    ...
    where
        K: AsRef<str>,
    {
        self.contracts.get(contract_key.as_ref()).clone()
    }

    pub fn get_by_address(&self, address: &H160) -> Option<ResolvedContract> {
        if let Some(key) = self.address_map.get(address) { self.get(key) } else { None }
    }
