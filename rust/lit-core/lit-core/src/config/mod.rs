...
use crate::error::{Result, config_err, io_err, ipfs_err, unexpected_err, validation_err};
...
impl LitConfig {
    ...
    pub fn admin_key(&self) -> Result<String> {
        // Order of precedence:
        // 1. Prefer admin.key from config if set (needed for `recreate`)
        // 2. Use LIT_ADMIN_KEY env var if set
        // 3. prompt user for input
        let admin_key = self.get_string("admin.key");
        #[cfg(feature = "cli")]
        let admin_key = admin_key.or_else(|_| {
            std::env::var("LIT_ADMIN_KEY")
                .map_err(|_| config_err("Admin key not found", None))
                .or_else(|_| read_sensitive_stdin("admin key", Some(64), true))
        });
        admin_key.map(remove_0x_prefix)
    }

    #[inline]
    pub fn wallet_key(&self, wallet_key_cfg_key: &str) -> Result<String> {
        // Order of precedence:
        // 1. Use wallet key from config if set
        // 2. Use LIT_WALLET_KEY env var if set (CLI only)
        // 3. Prompt user for input (CLI only)
        let wallet_key = self.get_string(wallet_key_cfg_key);
        #[cfg(feature = "cli")]
        let wallet_key = wallet_key.or_else(|_| {
            std::env::var("LIT_WALLET_KEY")
                .map_err(|_| config_err("Wallet key not found", None))
                .or_else(|_| read_sensitive_stdin("private key to node wallet", Some(64), true))
        });
        wallet_key.map(remove_0x_prefix)
    }

    #[inline]
    pub fn coms_sender_key(&self) -> Result<String> {
        // Order of precedence:
        // 1. Use coms.sender.key from config if set
        // 2. Use LIT_COMS_SENDER_KEY env var if set (CLI only)
        // 3. Prompt user for input (CLI only)
        let sender_key = self.get_string("coms.sender.key");
        #[cfg(feature = "cli")]
        let sender_key = sender_key.or_else(|_| {
            std::env::var("LIT_COMS_SENDER_KEY")
                .map_err(|_| config_err("Coms sender key not found", None))
                .or_else(|_| read_sensitive_stdin("coms sender private key", Some(64), true))
        });
        sender_key.map(remove_0x_prefix)
    }

    #[inline]
    pub fn coms_receiver_key(&self) -> Result<String> {
        // Order of precedence:
        // 1. Use coms.receiver.key from config if set
        // 2. Use LIT_COMS_RECEIVER_KEY env var if set (CLI only)
        // 3. Prompt user for input (CLI only)
        let receiver_key = self.get_string("coms.receiver.key");
        #[cfg(feature = "cli")]
        let receiver_key = receiver_key.or_else(|_| {
            std::env::var("LIT_COMS_RECEIVER_KEY")
                .map_err(|_| config_err("Coms receiver key not found", None))
                .or_else(|_| read_sensitive_stdin("coms receiver private key", Some(64), true))
        });
        receiver_key.map(remove_0x_prefix)
    }
...
mod tests {
    ...
    use crate::config::envs::LitEnv;
    #[allow(unused_imports)]
    use crate::config::{DEFAULT_IPFS_API, DEFAULT_IPFS_GATEWAY, LitConfigBuilder};
