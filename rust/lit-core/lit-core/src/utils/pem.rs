...
use crate::error::{Result, conversion_err, parser_err};
...
pub fn pem_to_der(pem: &str, guard: Option<&PemGuard>) -> Result<Vec<u8>> {
    ...
    STANDARD.decode(body).map_err(|e| parser_err(e, Some("failed to decode base64".into())))
}
...
pub fn der_to_pem<T: ?Sized + AsRef<[u8]>>(der: &T, guard: &PemGuard) -> String {
    ...
    let b64_string = STANDARD.encode(der.as_ref());
...
mod tests {
    ...
    fn ec_pub_pem_to_bytes_test() {
        ...
        assert_eq!(
            bytes_to_hex(&res),
            "04ee0b2578a435fd327c803698bb7c75e463f92629e0b25189ff37396b6a5c656077d25f4d7fa23fb28196430b6e238e80a50cd05b775e8513cd2045222c9a4fd6"
        );
    }
