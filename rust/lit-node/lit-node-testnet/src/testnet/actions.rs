...
impl Actions {
    ...
    pub async fn update_all_complaint_configss(&self, interval_secs: Option<u64>, tolerance: Option<u64>, kick_penalty_percent: Option<u64>, kick_penalty_demerits: Option<u64>) -> Result<()> {
            info!("Updating all complaint reason configs interval_secs to {:?} and tolerance to {:?}", interval_secs, tolerance);
            ...
            for i in 0..=MAX_COMPLAINT_REASON_VALUE {
                ...
                let current_config: lit_blockchain::contracts::staking::ComplaintConfig = self.contracts.staking
                ...
                let cc = self.contracts.staking.set_complaint_config(
                    ...
                    lit_blockchain::contracts::staking::ComplaintConfig {
                        ...
                        kick_penalty_percent: kick_penalty_percent.unwrap_or(current_config.kick_penalty_percent),  
                        kick_penalty_demerits: kick_penalty_demerits.unwrap_or(current_config.kick_penalty_demerits),
                    },
                );  
                if !Contracts::process_contract_call(
                    ...
                    .await {
                    return Err(anyhow::anyhow!("Error updating complaint config for reason {:?}", reason.as_u64()));
                }
            }
            Ok(())
        }
