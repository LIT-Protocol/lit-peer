...
pub(crate) enum EC {
    ...
    /// auth_sig signedMessage couldn't be converted to SessionKeySignedMessageV2
    #[code(kind = Validation, http_status = 400)]
    ...
    /// Old version of the endpoint is no longer supported
    #[code(kind = Validation, http_status = 410)]
    OldEndpointVersionNoLongerSupported,
    /// Payment failed
    #[code(kind = Validation, http_status = 402)]
    PaymentFailed,
    // Job not found
    #[code(kind = Validation, http_status = 404)]
    NodeJobNotFound,
    // The peer has provided incorrect information for key share validation
    #[code(kind = Validation, http_status = 400)]
    IncorrectInfoForKeyShareValidation,
}
