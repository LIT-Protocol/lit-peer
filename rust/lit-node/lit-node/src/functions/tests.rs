...
use lit_actions_server::{TestServer, init_v8};
...
use super::{ActionJob, ActionStore, ActionWorker, JobStatus};
use crate::models;
...
async fn auth_context(server: TestServer) {
    ...
    let code = indoc! {r#"
        ...
        console.log(Lit.Auth.actionIpfsIdStack)
    "#};
    ...
    assert_eq!(
        res.response,
        r#"{"actionIpfsIdStack":["some-ipfs-id"],"authSigAddress":null,"authMethodContexts":[],"resources":[],"customAuthResource":""}"#
    );
...
async fn call_child(server: TestServer) {
    ...
    let child1_ipfs_id = "QmYRKz97sdQ2DwG4w3uoTKSzycX2DDGQvuUQJuZN1UFdDR";
    let child1_code = indoc! {r#"
        (async () => {
            console.log("child action #1, actionIpfsIdStack:", LitAuth.actionIpfsIdStack.join(","))
            if (typeof ipfsId === "string") {
                await LitActions.call({ipfsId, params: {Link}})
            }
        })()
    "#};
    let child2_ipfs_id = "QmSbDFy9uhtLGAhBhMUC52jZruxuPnwsnR1qQNRLEU67GU";
    let child2_code = indoc! {r#"
        (async () => {
            console.log("child action #2, actionIpfsIdStack:", LitAuth.actionIpfsIdStack.join(","))
    ...
    assert_eq!(
        ...
        ExecutionState {
            ...
            logs: [
                "start",
                // Call #2
                &format!("child action #2, actionIpfsIdStack: {child2_ipfs_id}"),
                // Call #1, which calls #2
                &format!("child action #1, actionIpfsIdStack: {child1_ipfs_id}"),
                &format!("child action #2, actionIpfsIdStack: {child1_ipfs_id},{child2_ipfs_id}"),
                // Call #1, which calls itself
                &format!("child action #1, actionIpfsIdStack: {child1_ipfs_id}"),
                &format!("child action #1, actionIpfsIdStack: {child1_ipfs_id},{child1_ipfs_id}"),
                "end",
            ]
            .join("\n")
                + "\n",
...
async fn catch_op_errors(server: TestServer) {
...

    {
        ...
        assert_eq!(
            res.response,
            "Error: Console.log is printing something that is too long. Max length for all logs in a single request is 5 bytes"
        );
    }
}
...
async fn timeout(server: TestServer) {
    ...
    assert_eq!(
        ...
        "timeout error: tonic deadline exceeded error: Your function exceeded the maximum runtime of 500ms and was terminated."
    );
}
...
async fn oom(server: TestServer) {
    ...
    assert_eq!(
        res.unwrap_err().to_string(),
        "memory limit exceeded: tonic resource exhausted error: Your function exceeded the maximum memory of 100 MB and was terminated."
    );
}
...
async fn server_down() {
    ...
    assert_eq!(res.unwrap_err().kind(), lit_core::error::Kind::Unexpected);
}
...
#[rstest]
#[tokio::test]
async fn action_job(server: TestServer) {
    let client = Client::new(server.socket_path());

    let mut job = ActionJob::new(client, r#"console.log("invoked")"#);
    assert_eq!(job.state().logs, "");

    let res = job.run().await.unwrap();
    assert_eq!(res.logs, "invoked\n");
    assert_eq!(res.logs, job.state().logs);
}

#[rstest]
#[tokio::test]
async fn async_actions(server: TestServer) {
    let client = Client::new(server.socket_path());
    let mut store = ActionStore::new_in_memory().await.unwrap();

    let job_id = client
        .execute_js_async(r#"console.log("invoked")"#, &store)
        .await
        .unwrap();
    ...
    let failed_job_id = client.execute_js_async("boom", &store).await.unwrap();
    ...
    ActionWorker::new(store.clone(), Default::default())
        .start_with_shutdown(signal)
        .await
        .unwrap();
    ...
    // Failed job
    let job = store.get_job(&failed_job_id).await.unwrap().unwrap();
    assert_eq!(job.id, failed_job_id);
    ...
    assert_eq!(
        job.result
            ...
            .next()
            .unwrap(),
        "unexpected error: Uncaught ReferenceError: boom is not defined"
    );
    assert!(job.completed_at.is_some());

    // Wipe jobs
    let deleted_jobs = store.delete_all_jobs().await.unwrap();
