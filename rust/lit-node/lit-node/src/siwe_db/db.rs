...
use crate::{
    error::{EC, Result, conversion_err_code, unexpected_err, unexpected_err_code},
    models::EthBlock,
};
...
pub async fn init_fill_db(
    port: u16,
    quit_rx: mpsc::Receiver<bool>,
    http_client: reqwest::Client,
) -> Result<()> {
    let blocks: Vec<EthBlock> = http_client
        .get(INDEXER_URL)
        .timeout(Duration::from_secs(60))
        .send()
        .instrument(debug_span!("fetch_indexer_blocks"))
        .await
        .map_err(|e| {
            unexpected_err(
                e,
    ...
    db_batch_write(&mut conn, &blocks)?;
...
mod siwe_db_tests {
    ...
    use crate::{
        siwe_db::{
            db::{
                EthBlock, db_batch_write, db_conn, db_initial_setup, fetch_and_store_block_info,
                init_fill_db, retrieve_and_store_blockhash,
            },
