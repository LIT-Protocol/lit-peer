...
use crate::{
    ...
    utils::{encoding::hex_to_bytes, networking::get_web_addr_from_chain_info},
};
...
impl PeerState {
    ...
    pub async fn get_recovery_dkg_peers(&self) -> Result<SimplePeerCollection> {
        ...
        let recovery_peer_addresses = self
            ...
            .await
            .map_err(|e| {
                let reason = decode_revert(&e, self.backup_recovery_contract.abi());
                blockchain_err(reason, Some("Unable to get Recovery peer addresses".into()))
            })?;
        ...
        for (index, staker_address) in recovery_peer_addresses.iter().enumerate() {
            let validator: Validator = self
                .staking_contract
                .validator_by_staker_address(*staker_address)
                .call()
                .await
                .map_err(|e| {
                    blockchain_err(
            ...
            recovery_peers.push(SimplePeer {
                ...
                kicked: false,
                staker_address: peer.staker_address, // Not used for Recovery DKG
                key_hash: peer.key_hash,             // Not used for Recovery DKG
                version: crate::version::get_version(),
                realm_id: self.peers().realm_id()?,
            });
        }

        Ok(recovery_peers.into())
    }
