...
mod tests {
    ...
    use libsecp256k1::PublicKey;
    use lit_node_common::coms_keys::KeyPair;
    use xor_name::XorName;
    ...
    fn make_fake_peer(i: usize) -> PeerItem {
        ...
        let sender_public_key = KeyPair::generate().public_key().to_bytes();
        let receiver_public_key = KeyPair::generate().public_key().to_bytes();
    ...
    #[test]
    fn serialize_peer_item() {
        let peer = make_fake_peer(0);
        let mut bytes = Vec::with_capacity(1024);
        ciborium::into_writer(&peer, &mut bytes).unwrap();
        let peer2: PeerItem = ciborium::from_reader(&bytes[..]).unwrap();
