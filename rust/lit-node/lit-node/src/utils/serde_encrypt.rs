...
fn get_receiver_pubkey(peer_state: &PeerState, peer_addr: &str) -> Result<PublicKey> {
...
#[instrument(level = "debug", skip_all)]
fn get_sender_pubkey(peer_state: &PeerState, sender_id: XorName) -> Result<PublicKey> {
    let peer_state_data = peer_state.union_data.load_full();
...
#[instrument(level = "debug", skip_all)]
pub fn deserialize_and_decrypt<T: DeserializeOwned>(
    peer_state: &PeerState,
    sender_id: XorName,
    data: &[u8],
) -> Result<T> {
