...
use crate::error::{
    EC, Result, blockchain_err, blockchain_err_code, config_err, conversion_err,
    conversion_err_code, parser_err_code, serializer_err_code, unexpected_err, unexpected_err_code,
    validation_err, validation_err_code,
};
use crate::functions::action_client;
use crate::models::PoapEntry;
use crate::utils::encoding;
use lit_node_core::{
    AccessControlBooleanOperator, AccessControlConditionItem, AuthSigItem, ControlConditionItem,
    EndpointVersion, JsonAccessControlCondition, JsonAccessControlConditionOperator, JsonAuthSig,
    LitResourceAbility,
};
...
pub(crate) async fn check_access_control_conditions(
    ...
    let validate_res = JsonAuthSigExtendedRef::from(auth_sig)
...
async fn check_condition(
    ...
    if !condition.standard_contract_type.is_empty() {
        if condition.standard_contract_type == "POAP" {
            check_condition_via_poap(condition, auth_sig, http_client).await
...
async fn get_poaps_for_user(
    ...
    let result = http_client
        ...
        .send()
        .instrument(debug_span!("get_poaps_for_user"))
        .await
        .map_err(|e| {
            blockchain_err_code(e, EC::NodeRpcError, Some("Error making RPC Call".into()))
        })?;
...
async fn check_condition_via_poap(
    ...
    if condition.method == "tokenURI" {
        let poaps = get_poaps_for_user(condition, auth_sig, http_client).await?;
        for poap in &poaps {
...
async fn check_condition_via_lit_action(
    ...
    let code_from_ipfs = get_ipfs_file(
        &ipfs_id.to_string(),
        &cfg,
        ipfs_cache.clone(),
        http_client.clone(),
...
async fn check_condition_via_contract_call(
    ...
    if condition.standard_contract_type == "ERC1155" {
        abi = include_bytes!("../../config/abis/ERC1155.json").to_vec();
    } else if condition.standard_contract_type == "ERC721"
        || condition.standard_contract_type == "ERC721MetadataName"
    {
        abi = include_bytes!("../../config/abis/ERC721.json").to_vec();
    } else if condition.standard_contract_type == "ERC20" {
        abi = include_bytes!("../../config/abis/ERC20.json").to_vec();
    } else if condition.standard_contract_type == "MolochDAOv2.1" {
        abi = include_bytes!("../../config/abis/MolochDAOv2_1.json").to_vec();
    } else if condition.standard_contract_type == "CASK" {
        abi = include_bytes!("../../config/abis/CASK.json").to_vec();
    } else if condition.standard_contract_type == "Creaton" {
        abi = include_bytes!("../../config/abis/Creaton.json").to_vec();
    } else if condition.standard_contract_type == "ProofOfHumanity" {
        abi = include_bytes!("../../config/abis/ProofOfHumanity.json").to_vec();
    } else if condition.standard_contract_type == "PKPPermissions" {
        abi =
            include_bytes!("../../../../lit-core/lit-blockchain/abis/PKPPermissions.json").to_vec();
    } else {
    ...
    if condition.standard_contract_type == "PKPPermissions" {
        ...
        return if condition.method == "balanceOf" {
            ...
            if token_ids.len() != account_addresses.len() {
                warn!(
                    "Parameter mismatch: the number of token ids does not match the number of account addresses"
                );
                return Ok(false);
            }
        ...
        return if condition.method == "members" {
            ...
            */
            ...
            Ok(member.3 && member.5 == U256([0, 0, 0, 0]))
        } else {
            warn!("Unsupported method for contract ABI");
            Ok(false)
        };
        ...
        return if condition.method == "getActiveSubscriptionCount" {
            ...
            check_return_value_int(condition, sub_count)
        } else {
            warn!("Unsupported method for contract ABI");
            Ok(false)
        };
...
fn check_return_value_array(
    ...
    trace!(
        "Testing: Is {:?} {:?} {:?}",
        returned_values, condition.return_value_test.comparator, valid_return_value
    );
...
pub async fn substitute_special_params(
    ...
    if param == ":userAddress" {
        return JsonAuthSigExtendedRef(auth_sig)
            .user_address(bls_root_pubkey)
            .await;
...
#[cfg(test)]
mod tests {
    use crate::access_control::validate_boolean_expression;
    use lit_node_core::{
