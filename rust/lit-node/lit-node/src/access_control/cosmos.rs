...
use crate::error::{
    EC, Result, blockchain_err_code, serializer_err_code, unexpected_err_code, validation_err_code,
};
use chrono::{DateTime, Utc};
use lit_core::error::Unexpected;
use lit_node_core::{
    AccessControlBooleanOperator, CosmosBlock, CosmosCondition, CosmosConditionItem, JsonAuthSig,
    MultipleAuthSigs,
};
...
pub async fn check_condition(
    ...
    if condition.method.is_some()
        && condition
            ...
            == "timelock"
    {
        return check_condition_timelock(condition, auth_sigs, http_client).await;
    }
    ...
    let resp = http_client
        .get(&url)
        .send()
        .instrument(debug_span!("fetch_cosmos"))
        .await;
...
pub async fn get_timestamp_from_block_height(
    url: &str,
    http_client: reqwest::Client,
) -> Result<i64> {
    let response = http_client
        .get(url)
        .send()
        .instrument(debug_span!("get_timestamp_from_block_height"))
        .await
        .map_err(|e| {
            unexpected_err_code(
                e,
...
mod tests {
    ...
    use lit_node_core::{
        CosmosCondition, JsonAuthSig, JsonReturnValueTestV2, MultipleAuthSigs,
