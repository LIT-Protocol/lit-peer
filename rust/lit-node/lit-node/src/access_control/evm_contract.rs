...
use lit_node_core::{
    AccessControlBooleanOperator, EVMContractCondition, EVMContractConditionItem, EndpointVersion,
    JsonAuthSig, LitResourceAbility,
};
...
pub async fn check_condition(
    ...
    match returned_value {
        ...
        .add_source_to_details()),
        Token::Tuple(s) => Err(validation_err_code(
            "Tuple not supported",
            EC::NodeInvalidConditionTokenType,
            None,
        )
...
//         return Ok(returned_value != valid_return_value);
//     } else {
//         debug!("Error - unsupported return value test comparator");
//         return Ok(false);
//     }
// }
