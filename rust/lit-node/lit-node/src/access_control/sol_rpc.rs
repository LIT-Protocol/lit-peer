...
use crate::error::{
    EC, Result, blockchain_err_code, conversion_err, parser_err, unexpected_err_code,
    validation_err_code,
};
use lit_core::error::Unexpected;
use lit_node_core::{
    AccessControlBooleanOperator, JsonAuthSig, SolPdaInterface, SolRpcConditionItem,
    SolRpcConditionV2, SolRpcConditionV2Options,
};
...
pub async fn check_condition(
        ...
        match resp {
            Ok(parsed_response) => {
                ...
                match parsed_response["value"].clone() {
                    ...
                    _ => {
                        return Err(unexpected_err_code(
                            "Error making RPC Call getAccountInfo in getPDA - couldn't parse result into an object",
                            EC::NodeRpcError,
                            None,
                        ));
                    }
    ...
    let sol_client = get_sol_rpc_client(&condition.chain)?;
    let rpc_request = match rpc_method.as_str() {
        "getBlock" => RpcRequest::GetBlock,
        "getBalance" => RpcRequest::GetBalance,
        "getAccountInfo" => RpcRequest::GetAccountInfo,
        "getTokenAccountBalance" => RpcRequest::GetTokenAccountBalance,
        "getTokenAccountsByDelegate" => RpcRequest::GetTokenAccountsByDelegate,
        "getTokenAccountsByOwner" => RpcRequest::GetTokenAccountsByOwner,
...
fn check_balance_of_metaplex_collection(
    ...
    //     debug!("collection_params: {:?}", collection_params);
    //     collection_resp = sol_client.send(RpcRequest::GetAccountInfo, collection_params);
    // } else {
    //     debug!("Error, collection address is not a string");
    ...
    //                     debug!("before decoding base64: {:?}", val);
    //                     let decoded =
    ...
    //                                 debug!("Collection isn't verified.  Aborting");
    //                                 return Ok(false);
    //                             }
    //                         }
    //                         Err(err) => {
    //                             debug!("Error parsing collection: {:?}", err);
    //                             return Err(anyhow!("rpc_error"));
    //                         }
    //                     }
    //                 }
    ...
    //     }
    //     Err(err) => {
    //         debug!("Error making RPC Call: {:?}", err);
    //         return Err(anyhow!("rpc_error"));
    //     }
    // }
    ...
    //     Ok(program_accounts) => {
    //         debug!("program accounts: {:?}", program_accounts);
    //     }
    //     Err(err) => {
    //         debug!("Error making RPC Call: {:?}", err);
    //         return Err(anyhow!("rpc_error"));
...
mod tests {
    ...
    use lit_node_core::{
        JsonAuthSig, JsonReturnValueTestV2, SolPdaInterface, SolRpcConditionV2Options,
