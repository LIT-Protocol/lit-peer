...
impl AuthMethodVerifier for AppleJwtAuthMethodVerifier {
    async fn verify(
        ...
        get_apple_auth_from_jwt(access_token, http_client).await
    }
}

pub async fn save_apple_keys(http_client: reqwest::Client) -> Result<Value> {
    trace!("Downloading Apple keys...");
    let url = "https://appleid.apple.com/auth/keys";
    let resp = http_client
        .get(url)
        .send()
        .instrument(debug_span!("fetch_apple_keys"))
        .await
...
pub async fn get_apple_keys(http_client: reqwest::Client) -> Result<Value> {
    ...
    let contents = match fs::read_to_string(APPLE_KEYS_FILE_PATH) {
        ...
        Err(e) => {
            if e.kind() == std::io::ErrorKind::NotFound {
                ...
                return save_apple_keys(http_client).await;
            } else {
                return Err(unexpected_err(
                    e,
...
pub async fn get_apple_auth_from_jwt(
    ...
    let apple_keys = get_apple_keys(http_client)
        .await
