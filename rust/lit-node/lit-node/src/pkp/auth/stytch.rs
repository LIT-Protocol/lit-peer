...
const AUTHORIZATION_KEYS_FILE_DIR_PATH: &str = "./node_state/stytch";
...
impl AuthMethodVerifier for StytchJWTAuthMethodVerifier {
    async fn verify(
        ...
        return get_otp_auth_from_jwt(access_token, self.factor.clone(), http_client).await;
    }
}
...
pub async fn get_otp_auth_from_jwt(
    ...
    let auth_keys = get_auth_key(token, http_client).await;
...
pub async fn get_auth_key(token: &str, http_client: reqwest::Client) -> error::Result<Value> {
    ...
    if let Err(e) = contents {
        if e.kind() == std::io::ErrorKind::NotFound {
            ...
            return save_auth_key(project_id.0.as_str(), project_id.1.as_str(), http_client).await;
        } else {
            return Err(io_err(
                e,
    ...
    match last_retrieved_at {
        Some(at) => {
            ...
            if last_retrieved_at < one_hour_ago {
                ...
                // get the keys again
                return save_auth_key(token, project_id.1.as_str(), http_client).await;
            }
        }
        None => {
            // get the keys again
...
pub async fn save_auth_key(
    ...
    let resp = http_client
        .get(url)
        .send()
        .instrument(debug_span!("fetch_stytch_keys"))
        .await
...
mod tests {
    use crate::pkp::auth::stytch::{
        StytchJWTAuthMethodVerifier, get_auth_key, parse_and_verify_otp_jwt,
    };
