use crate::error::{self, Unexpected, conversion_err, parser_err, unexpected_err};
use crate::models;
use chrono::Days;
use std::str;
...
impl AuthMethodVerifier for DiscordAuthMethodVerifier {
    async fn verify(
        ...
        get_discord_auth_from_access_token(access_token, http_client).await
    }
}
...
pub async fn get_discord_auth_from_access_token(
    ...
    let res = http_client
        ...
        .headers(headers)
        .send()
        .instrument(debug_span!("fetch_discord_oauth"))
        .await
