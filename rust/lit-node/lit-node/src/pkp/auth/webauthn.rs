...
use crate::models;
use crate::{
    error::{EC, Result, parser_err, unexpected_err, validation_err_code},
    utils::encoding,
};
...
pub async fn check_blockhash_challenge<M: Middleware + 'static>(
    ...
    debug!("retrieved blockhash: {:?}", retrieved.blockhash);
    debug!(
        "latest stored blockhash: {:?}",
        latest_stored_block.blockhash
    );
    debug!("retrieved block number: {:?}", retrieved.block_number);
    debug!(
...
mod tests {
    ...
    use lit_node_core::constants::CHAIN_ETHEREUM;
