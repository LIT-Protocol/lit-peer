use crate::error::{
    self, Result, Unexpected, conversion_err, parser_err, unexpected_err, validation_err,
};
use crate::models;
...
const GOOGLE_KEYS_FILE_PATH: &str = "./node_state/oauth/google_keys.json";
...
impl AuthMethodVerifier for GoogleAuthMethodVerifier {
    async fn verify(
        ...
        get_google_auth_from_access_token(access_token, http_client).await
    }
}
...
impl AuthMethodVerifier for GoogleJwtAuthMethodVerifier {
    async fn verify(
        ...
        get_google_auth_from_jwt(access_token, http_client).await
    }
}

pub async fn save_google_keys(http_client: reqwest::Client) -> Result<Value> {
    trace!("Downloading Google keys...");
    let url = "https://www.googleapis.com/oauth2/v3/certs";
    let resp = http_client
        .get(url)
        .send()
        .instrument(debug_span!("fetch_google_certs"))
        .await
...
pub async fn get_google_keys(http_client: reqwest::Client) -> Result<Value> {
    ...
    let contents = match fs::read_to_string(GOOGLE_KEYS_FILE_PATH) {
        ...
        Err(e) => {
            if e.kind() == std::io::ErrorKind::NotFound {
                ...
                return save_google_keys(http_client).await;
            } else {
                return Err(unexpected_err(
                    e,
...
pub async fn get_google_auth_from_jwt(
    ...
    let google_keys = get_google_keys(http_client)
        .await
...
pub async fn get_google_auth_from_access_token(
    ...
    let res = http_client
        ...
        .headers(headers)
        .send()
        .instrument(debug_span!("fetch_google_oauth"))
        .await
        .map_err(|e| {
            unexpected_err(
                e,
