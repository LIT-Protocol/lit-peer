...
use crate::error::{
    EC, EC::NodePKPNotAuthorized, EC::NodeUnknownError, Result, conversion_err, unexpected_err,
    unexpected_err_code,
};
...
use lit_node_core::SignableOutput;
use lit_node_core::SigningScheme;
use lit_node_core::request::JsonPKPClaimKeyRequest;
use lit_node_core::response::JsonPKPClaimKeyResponse;
...
#[instrument(level = "debug", skip(tss_state, cfg, auth_sig, auth_context))]
pub async fn sign(
    cfg: &LitConfig,
    to_sign: &[u8],
    pubkey: String,
    ...
    let (tweak_preimage, root_pubkeys) = match tweak_preimage {
        ...
        Err(_) => {
            ...
            debug!("op_sign() any_key_share_exists() result: {:?}", &result);

            match result {
                Ok(Some((curve_type, _))) => {
                    if !signing_scheme.supports_curve(curve_type) {
                        trace!(
                            "op_sign() {}.supports_curve({}) mismatch",
                ...
                Err(err) => {
                    trace!("op_sign() any_key_share_exists() error: {:?}", &err);
                    return Err(unexpected_err_code(
                        err,
                        NodeUnknownError,
                        Some(format!(
                ...
                Ok(None) => {
                    trace!(
                        "op_sign() pubkey share not found on this node PKP: {}",
                        pubkey
                    );
                    return Err(unexpected_err_code(
...
pub async fn claim_key(
    ...
    // first auth the user with their given auth method, there should only be one for a context-less derivation scheme
    ...
    let permissions_contract = resolver.pkp_permissions_contract(&cfg).await.map_err(|e| {
        unexpected_err(e, Some("Could not resolve pkp permissions contract".into()))
    })?;
    ...
    trace!("auth context serialized: {:?}", &key_id);
