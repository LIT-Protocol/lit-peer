    ...
    async fn validate_capability_auth_sig(
        ...
        match auth_sig.auth_material_type {
            ...
            _ => {
                return Err(validation_err_code(
                    format!(
                        "The auth sig material type {:?} is not supported when calling validate_auth_sig() within session sig validation.  Only WalletSig and BLSNetworkSig are supported.",
                        auth_sig.auth_material_type
                    ),
                    EC::NodeSIWEMessageError,
                    None,
                ));
...
impl SessionSigAuthSigValidator for SiweValidator {
    ...
    async fn validate_bls_auth_sig_basic(
        ...
        .expect("Failed to convert bls public key from bytes");
        debug!(
            "bls session signature verify: bls_root_key: {}, signed_data: {:?}",
            bls_root_key, signed_data
        );
        match signature.verify(&bls_root_key, signed_data) {
            Ok(_) => {}
            Err(e) => {
    ...
    async fn validate_auth_sig(
        ...
        for session_capability_object in &session_capability_objects {
            ...
            {
                debug!(
                    "Failed to verify capability object - this may not be an issue, since we check all of them, and only need 1 to pass: {}",
                    e
                );
                errors.push(e);
            } else {
...
mod tests {
    ...
    use crate::{error::EC, utils::encoding};
    use lit_node_core::{AccessControlConditionResource, JsonAuthSig};
