use crate::error::{EC, Result, conversion_err, parser_err, parser_err_code, validation_err_code};
use crate::utils::encoding;
use ethers::types::Address;
use lit_node_core::{
    JsonAuthSig,
    constants::{CHAIN_CHEQD, CHAIN_COSMOS, CHAIN_EVMOS, CHAIN_JUNO, CHAIN_KYVE},
};
    ...
    for i in 0..4 {
        ...
        match libsecp256k1::recover(&message, &signature, &recovery_id) {
            Ok(pubkey) => {
                ...
                let address = bech32::encode::<bech32::Bech32>(
                    bech32::Hrp::parse(chain_derivation_prefix.as_str())
                        .map_err(|e| parser_err(e, None))?,
                    &address_before_bech32,
                )
                .map_err(|e| parser_err_code(e, EC::NodeParserError, None))?;
...
pub fn get_evm_address(address: &str) -> Result<Address> {
    let (_hrp, bytes) = bech32::decode(address)
