...
use super::resources::{get_resource_prefix_id_from_type, parse_resource_and_prefix};
use super::validators::auth_sig::SessionSigAuthSigValidator;
use super::validators::siwe::SiweValidator;
use lit_node_core::{AuthMaterialType, JsonAuthSig, LitResource, LitResourceAbility};
...
pub(crate) fn extract_requested_resources_from_session_sig(
    ...
    let session_key_signed_message = parse_signed_message(&session_sig.signed_message)?;
...
async fn validate_capabilities_array<'a>(
    ...
    for inner_auth_sig in capabilities {
        ...
        match validation_res {
            ...
            Err(e) => {
                debug!(
                    "When searching for delegation auth sig for a session sig, the capability auth sig validation failed.  This may not be a problem, since we check all capabilities and another one might be valid.  Printing the error here just in case: {:?}",
                    e
                );
            }
        }
...
// Parse session key signed message
fn parse_signed_message(signed_message: &str) -> Result<SessionKeySignedMessageV2> {
    serde_json::from_str::<SessionKeySignedMessageV2>(signed_message)
        .map_err(|e| conversion_err_code(e, EC::NodeAuthSigSignedMessageConversionError, None))
}
...
mod validate_session_sig_tests {
    ...
    async fn test_validate_success() {
        ...
        match validate {
            ...
            Err(e) => {
                debug!("Failed to validate session sig: {:?}", e);
                assert!(false);
                return;
            }
        }
