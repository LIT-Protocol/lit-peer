use crate::error::{EC, Result, parser_err, parser_err_code, validation_err_code};
use iri_string::spec::UriSpec;
use lit_node_core::{LitAbility, LitResource, LitResourceAbility, ResourceType};
...
pub fn extract_and_verify_all_capabilities(
    ...
    trace!("Resources length: {}", siwe_message.resources.len());
    ...
    let capabilities = siwe_message
        ...
        .collect::<Vec<_>>();
    trace!("Capabilities length: {}", capabilities.len());
    ...
    fn verify_capabilities_for_resource(
        ...
        let attenuated_resource_key = {
            match requested_lit_resource_ability.get_resource() {
                ...
                ResourceType::PaymentDelegation(r) => {
                    self.get_resource_key_to_match_against(r.to_owned())?
                }
            }
        };
...
pub(crate) fn get_recap_namespace_and_ability(
    ...
    match lit_ability {
        ...
        LitAbility::PaymentDelegationAuth => Ok((LitRecapNamespace::Auth, LitRecapAbility::Auth)),
    }
}
...
mod extract_and_verify_tests {
    ...
    use lit_node_core::{AccessControlConditionResource, LitActionResource, LitResourcePrefix};
    ...
    fn test_extract_and_verify_with_two_recaps_in_one_resource() {
        ...
        debug!("{:?}", session_capability_objects);
        ...
        let session_capability_object = session_capability_objects.first().unwrap();
...
mod recap_tests {
    ...
    use lit_node_core::AccessControlConditionResource;
