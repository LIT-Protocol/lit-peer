...
use crate::common::networking::{ClientProxyConfiguration, get_local_url_from_port};

use lit_node::networking::grpc::client::get_grpc_url_from_http_url;
...
pub fn generate_and_save_proxy_mappings_for_local_testing(
    ...
    for i in 0..num_nodes {
        ...
        for j in 0..num_nodes {
            let dest_port = initial_port + j;
            let proxy_grpc_url =
                ClientProxyConfiguration::determine_local_proxy(source_port, dest_port);

            let dest_grpc_url = get_grpc_url_from_http_url(get_local_url_from_port(dest_port));
            debug!(
...
pub fn setup_proxies(proxy_mappings: &'static ClientProxyMapping) {
    thread::spawn(|| {
        ...
        trace!(
            "Setting up {} proxies using toxiproxy.",
            proxy_mappings.proxy_mappings().len()
        );
        ...
        for (our_url, dest_urls) in proxy_mappings.proxy_mappings().iter() {
            ...
            for (dest_url, proxy_url) in dest_urls.iter() {
                ...
                trace!(
                    "Setting up proxy {:?} from {}:{} to {}:{} at {}:{}",
                    proxy_name,
                    our_url.host_str().unwrap(),
                    our_url.port_or_known_default().unwrap(),
                    dest_url.host_str().unwrap(),
                    dest_url.port_or_known_default().unwrap(),
...
pub fn inject_fault_between_urls(fault_type: FaultType, url_1: &Url, url_2: &Url) {
    info!(
        ...
        fault_type,
        url_1.host_str().unwrap(),
        url_1.port_or_known_default().unwrap(),
        url_2.host_str().unwrap(),
        url_2.port_or_known_default().unwrap()
    );
...
pub fn inject_fault_between_all_sources_to_random_target(
    ...
    info!(
        ...
        fault_type,
        random_fault_node.host_str().unwrap(),
        random_fault_node.port_or_known_default().unwrap()
    );
    for i in 0..num_nodes {
    ...
    match fault_type {
        ...
        FaultType::LatencyAboveClientTimeout => {
            inject_latency_fault(
                ...
                target_url.clone(),
                Duration::seconds(
                    i64::try_from(FAULT_TEST_CHATTER_CLIENT_TIMEOUT_SECS + 5).unwrap(),
                )
                .num_milliseconds()
                .try_into()
                .unwrap(),
...
pub fn inject_latency_fault(
    ...
    info!(
        ...
        source_url.port_or_known_default().unwrap(),
    ...
    thread::spawn(move || {
        ...
        info!("Setting up latency fault for {:?}", proxy_name);
        ...
        info!("Latency fault set up for {:?}", proxy_name);
    })
    .join()
