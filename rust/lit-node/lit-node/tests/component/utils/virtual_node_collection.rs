...
use lit_node::{
    config::chain::{ChainDataConfigManager, PeersForEpoch},
    ...
    tasks::{presign_manager::models::PresignMessage, utils::generate_hash},
    tss::{
        common::{
            models::{NodeTransmissionDetails, RoundData},
            tss_state::TssState,
        },
        ecdsa_damfast::DamFastState,
    },
    version::get_version,
};
use lit_node_common::{
    coms_keys::ComsKeys,
    config::{LitNodeConfig, load_cfg},
    eth_wallet_keys::EthWalletKeys,
};
...
impl VirtualNode {
    pub fn damfast_state(&self, signing_scheme: SigningScheme) -> DamFastState {
        DamFastState::new(self.tss_state.clone(), signing_scheme)
    }
}
...
pub struct BgChannels {
    pub rx_node_transmission_details: TracedReceiver<NodeTransmissionDetails>,
...
impl VirtualNodeCollection {
    ...
    pub(crate) async fn update_internal_state(&mut self) {
        ...
        for (index, node) in self.nodes.iter_mut().enumerate() {
            ...
            node.peer.peer_id = PeerId::from_slice(
                &node
                    .tss_state
                    .peer_state
                    .wallet_keys
                    .verifying_key_uncompressed_array(),
            )
            .unwrap();
        }
    ...
    pub(crate) fn staker_addresses(&self) -> Vec<H160> {
        self.nodes.iter().map(|n| n.staker_address).collect()
    }

    pub(crate) async fn add_node_internal(&mut self, port: u16, staker_address: H160) -> bool {
        ...
        let validator = PeerValidator {
            ...
            wallet_public_key: tss_state
                .peer_state
                .wallet_keys
                .verifying_key_uncompressed_vec(),
            staker_address,
    ...
    pub async fn update_cdm_epoch(&mut self, epoch: u64) {
        for node in self.nodes.iter_mut() {
            let mut peers_for_current_epoch = DataVersionWriter::new_unchecked(
                &node
                    .tss_state
            ...
            peers_for_current_epoch.epoch_number = epoch;
            peers_for_current_epoch.commit();
        }
    }

    pub async fn update_cdm_realm_id(&mut self, realm_id: u64) {
        for node in self.nodes.iter_mut() {
            DataVersionWriter::store(
                &node.tss_state.chain_data_config_manager.realm_id,
                U256::from(realm_id),
            );
        }
    }
    ...
    pub async fn update_cdm_validators(&self) {
        ...
        for node in self.nodes.iter() {
            let mut peers_for_epoch = DataVersionWriter::new_unchecked(
                &node
                    .tss_state
                    .chain_data_config_manager
                    .peers
...
async fn load_virtual_node_defaults(
    port: u16,
    testnet: &lit_node_testnet::testnet::Testnet,
    staker_address: H160,
) -> (
    Arc<TssState>,
    Receiver<RoundData>,
    ...
    let (tss_state, rx_round_sender, rx_batch_sender) = TssState::init_channels(
        addr,
        peer_state,
        port as u32,
        Arc::new(cfg.clone()),
    ...
    assert!(tss_state.peer_state.get_tss_state().is_ok());
...
async fn new_peer_state(
    ...
    let resolver = ContractResolver::try_from(lit_config.load().as_ref())
        .expect("failed to load ContractResolver");

    let staking_contract = resolver
        .staking_contract_with_gas_relay(cfg.as_ref(), secret_key.clone())
        .await
        .expect("failed to load staking contract");
    ...
    let sender_privkey = cfg
        .coms_keys_sender_privkey()
        .expect("failed to get coms_keys_sender_privkey");
    let receiver_privkey = cfg
    ...
    PeerState {
        ...
        complaint_channel,
        chain_data_config_manager,
        comskeys: ComsKeys::from_node_config(sender_privkey, receiver_privkey)
            .expect("failed to get comskeys from node config"),
        wallet_keys: EthWalletKeys::random(&mut rand::rngs::OsRng),
        lit_config,
        ps_tx,
        tss_state: Weak::new(),
