...
pub async fn setup_background_channels(
    ...
    for node in vnc.nodes.iter_mut() {
        let rx_node_transmission_details = node.node_channels.rx_node_transmission_details.clone();
...
pub async fn data_receiver(
    ...
    loop {
        ...
        msg = rx_node_transmission_details.recv_async() => {
                ...
                let (chan_msg, _span) = msg.expect("TEST::background_channel_mapper: rx_node_transmission_details.recv_async().await is_err()");
                let transmission_details = chan_msg.data();
