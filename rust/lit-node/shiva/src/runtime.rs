...
use crate::{models::TestNetMessage, testnet_instance::TestnetInstance};
...
pub fn create_runtime(
    ...
    let testnet_instance = thread::spawn(move || {
        ...
        let spawner = tn_rt.block_on(async {
            let instance = TestnetInstance::init(params.clone()).await;
                    ...
                    handler_command = c_handler_rx.recv_async() => {
                        ...
                        match handler_command {
                            TestNetCommand::GetInfo(sender) => {
                                ...
                                let epoch_length = instance.epoch_length;
                            ...
                            TestNetCommand::StopRandomAndWait(sender) => {
                                let current_epoch = instance.validators.actions().get_current_epoch(realm_id).await;
                                let res = instance.stop_random_node().await;
                                if res.is_ok() {
                                    instance.validators.actions().wait_for_epoch(realm_id, current_epoch + 1).await;
                                    let _ = sender.send_async(true).await;
                                } else {
                                    let _ = sender.send_async(false).await;
                                }
