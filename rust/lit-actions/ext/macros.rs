    ...
    ($name:ident, $state:ident, $send:expr, $match:pat => $res:expr) => {{
        ...
        let (tx, rx) = {
            let tx = $state
                ...
                .clone();
            let rx = $state.borrow::<::lit_observability::channels::TracedReceiver<::lit_actions_grpc::proto::ExecuteJsRequest>>().clone();
            (tx, rx)
        };
    ...
    ($name:ident, $state:ident, $send:expr, $match:pat => $res:expr) => {{
        ...
        let (tx, rx) = {
            ...
            let tx = state
                ...
                .clone();
            let rx = state
                .borrow::<::lit_observability::channels::TracedReceiver<::lit_actions_grpc::proto::ExecuteJsRequest>>()
                .clone();
            (tx, rx)
        };
