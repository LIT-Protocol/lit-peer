    ...
    ERC2771
{
    ...
    function getPkpInfoFromOwnerAddress(
        address owner,
        uint256 pageSize,
        uint256 pageIndex
    ) public view returns (LibPubkeyRouterStorage.PkpInfo[] memory) {
        PKPNFTFacet pkpNft = PKPNFTFacet(address(this));
        uint256 balance = pkpNft.balanceOf(owner);

        uint256 startIndex = pageSize * pageIndex;
        ...
        for (uint256 i = startIndex; i < endIndex; i++) {
            tokenIds[i - startIndex] = pkpNft.tokenOfOwnerByIndex(owner, i);
        }
        return getPkpInfoFromTokenIds(tokenIds);
    }
    function getPkpInfoFromOwnerTokenId(
        uint256 tokenId,
        uint256 pageSize,
        uint256 pageIndex
    ) public view returns (LibPubkeyRouterStorage.PkpInfo[] memory) {
        PKPNFTFacet pkpNft = PKPNFTFacet(address(this));
        address owner = pkpNft.ownerOf(tokenId);
        return getPkpInfoFromOwnerAddress(owner, pageSize, pageIndex);
    }
