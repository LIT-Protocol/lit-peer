...
contract PKPHelper is Ownable, IERC721Receiver, AccessControl {
    ...
    struct MintNextAndAddAuthMethodsWithTypesParams {
        uint256 keyType;
        string keySetId;
        bytes[] permittedIpfsCIDs;
        uint256[][] permittedIpfsCIDScopes;
        address[] permittedAddresses;
        uint256[][] permittedAddressesScopes;
        uint256[] permittedAuthMethodTypes;
        bytes[] permittedAuthMethodIds;
        bytes[] permittedAuthMethodPubkeys;
        uint256[][] permittedAuthMethodScopes;
    ...
    function mintNextAndAddAuthMethods(
        ...
        MintNextAndAddAuthMethodsWithTypesParams
            memory params = MintNextAndAddAuthMethodsWithTypesParams({
                keyType: keyType,
                keySetId: keySetId,
                permittedIpfsCIDs: new bytes[](0), // permitted ipfs CIDs
                permittedIpfsCIDScopes: new uint256[][](0), // permitted ipfs CIDs scopes
                permittedAddresses: new address[](0), // permitted addresses
                permittedAddressesScopes: new uint256[][](0), // permitted addresses scopes
                permittedAuthMethodTypes: permittedAuthMethodTypes,
                permittedAuthMethodIds: permittedAuthMethodIds,
                permittedAuthMethodPubkeys: permittedAuthMethodPubkeys,
                permittedAuthMethodScopes: permittedAuthMethodScopes,
                addPkpEthAddressAsPermittedAddress: addPkpEthAddressAsPermittedAddress,
                sendPkpToItself: sendPkpToItself
            });
    ...
    */
    ...
    function mintNextAndAddAuthMethodsWithTypes(
        ...
        require(
            params.permittedIpfsCIDs.length ==
                params.permittedIpfsCIDScopes.length,
            "PKPHelper: ipfs cid and scope array lengths must match"
        );
        require(
            params.permittedAddresses.length ==
                params.permittedAddressesScopes.length,
    ...
    function claimAndMintNextAndAddAuthMethodsWithTypesV2(
        ...
        // mint the nft and forward the funds
        uint256 realmId = 1; // technically this shouldn't matter
        uint256 tokenId = PKPNFTFacet(getPkpNftAddress()).claimAndMint{
            value: msg.value
        }(
            realmId,
            claimMaterial.keyType,
            claimMaterial.keySetId,
            claimMaterial.derivedKeyId,
            claimMaterial.signatures,
