...
contract Multisender is Ownable {
    ...
    function sendTokens(
        ...
        uint256 val = bal / (_recipients.length * 10);
        for (uint256 i = 0; i < _recipients.length; i++) {
            tkn.transfer(_recipients[i], val);
        }
    }

    function sendTokensExact(
        address[] calldata _recipients,
        address tokenContract,
        uint256 amountPerRecipient
    ) public {
        ERC20 tkn = ERC20(tokenContract);
        uint256 totalAmount = amountPerRecipient * _recipients.length;
        require(
            tkn.balanceOf(address(this)) >= totalAmount,
            "Insufficient token balance"
        );
        for (uint256 i = 0; i < _recipients.length; i++) {
            tkn.transfer(_recipients[i], amountPerRecipient);
        }
    }
