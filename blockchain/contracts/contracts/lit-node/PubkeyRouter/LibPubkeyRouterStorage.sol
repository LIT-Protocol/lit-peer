...
library LibPubkeyRouterStorage {
    ...
    struct PubkeyRouterStorage {
        ...
        // map the keccak256(uncompressed pubkey) -> PubkeyRoutingData
        mapping(uint256 => PubkeyRoutingData) pubkeys;
        // map the eth address to a pkp id
        mapping(address => uint256) ethAddressToPkpId;
        // map staking contract to the keyset mapping to root keys
        mapping(address => mapping(bytes32 => IPubkeyRouter.RootKey[])) rootKeys;
    }
