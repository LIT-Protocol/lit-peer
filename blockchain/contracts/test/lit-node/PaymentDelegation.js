...
describe('PaymentDelegation', function () {
  ...
  it('allows delegating and undelegating payments', async () => {
    ...
    let [totalMaxPrice, requestsPerPeriod, periodSeconds] = restrictions[0][0];
    expect(totalMaxPrice).to.equal(0);
  ...
  it('allows setting restrictions', async () => {
    ...
    expect(deployerRestriction.totalMaxPrice).to.equal(
      restriction.totalMaxPrice
    );
  ...
  it('allows batch delegating and undelegating payments', async () => {
    ...
    let [totalMaxPrice, requestsPerPeriod, periodSeconds] = userRestrictions[0];
    expect(totalMaxPrice).to.equal(50);
    expect(requestsPerPeriod).to.equal(100);
    expect(periodSeconds).to.equal(3600);
    expect(anotherUserRestrictions.length).to.equal(1);
    [totalMaxPrice, requestsPerPeriod, periodSeconds] =
      anotherUserRestrictions[0];
    expect(totalMaxPrice).to.equal(50);
    expect(requestsPerPeriod).to.equal(100);
    expect(periodSeconds).to.equal(3600);
