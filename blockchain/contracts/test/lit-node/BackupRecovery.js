...
describe('BackupRecovery', function () {
  ...
  beforeEach(async function () {
    ...
    const { diamond: backupRecoveryDiamond } = await deployDiamond(
      ...
      Environment.DEV,
      {
        additionalFacets: ['BackupRecoveryFacet', 'BackupRecoveryViewsFacet'],
        verifyContracts: false,
        waitForDeployment: false,
      }
    );

    // load diamond ABI from json artifacts
    const artifact = await hre.artifacts.readArtifact(
      '@lit-protocol/hardhat-diamond-abi/HardhatDiamondABI.sol:BackupRecoveryDiamond'
    );
    backupRecoveryContract = new ethers.Contract(
    ...
    try {
      ...
      minStake = await stakingViewsFacet.minStake();
      const totalToStake = minStake * 10n; // 3 times the minimum stake
      ...
      for (let i = 0; i < backupPartyAccounts.length; i++) {
        await deployer.sendTransaction({
          ...
          value: ethers.parseEther('20.0'), // was 5.0
        });
      }
    } catch (e) {
      console.log('Error: ', e);
    ...
    console.log('registering new backup recovery party', backupAddresses);
  ...
  it('should not move nextState until all backup parties register keys', async () => {
    ...
    let sessionId = keccak256(ethers.getBytes('0x12345678'));

    const registeredRecoveryKeys = [
      {
        pubkey: ethers.getBytes(blsKey),
        keyType: 1,
      },
      {
        pubkey: ethers.getBytes(pubkey),
        keyType: 2,
      },
    ];
    ...
    for (let i = 0; i < backupPartyAccounts.length - 1; i++) {
      ...
      const tx = await backupContract.receiveNewKeySet(
        registeredRecoveryKeys,
        sessionId
      );
      await tx.wait();
    }
    ...
    expect(recState[1]).equal(0n);
    expect(recState[2]).to.deep.equal([]);
    expect(recState[3]).to.deep.equal([]);
  });
  ...
  it('should submit valid proof', async () => {
    ...
    const tx = await backupContract.receiveProofBls12381G1(proof);
    await tx.wait();
    let proofCount =
