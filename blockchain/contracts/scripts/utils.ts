...
import { BaseContract, Contract, Signer, Wallet, Provider } from 'ethers';
...
/// Maps an array of `WalletManifestItem` to an array of `NodeOperatorCredentials`.
export function walletManifestItemsToNodeOperatorCredentials(
  walletManifestItems: Array<WalletManifestItem>,
  provider: Provider
): Array<NodeOperatorCredentials> {
  const allWallets = walletManifestItems.map((walletManifestItem, idx) => {
    const nodeWallet = new Wallet(walletManifestItem.node.privateKey, provider);
    const stakerWallet = new Wallet(
      walletManifestItem.staker.privateKey,
      provider
    );
    ...
    return {
      ...
      stakerWallet,
      comsKeysSender: {
        publicKey: walletManifestItem.node.comsKeysSender.publicKey,
        privateKey: walletManifestItem.node.comsKeysSender.privateKey,
      },
      comsKeysReceiver: {
        publicKey: walletManifestItem.node.comsKeysReceiver.publicKey,
        privateKey: walletManifestItem.node.comsKeysReceiver.privateKey,
      },
    };
  });
  return allWallets;
...
/// Maps an array of `NodeOperatorCredentials` to an array of `WalletManifestItem`.
export function nodeOperatorCredentialsToWalletManifestItems(
  nodeOperatorsCredentials: Array<NodeOperatorCredentials>
): Array<WalletManifestItem> {
...
function generateNodeRuntimeConfig(
  ...
  return {
    ...
    node: {
      ...
      actions_socket: `/tmp/actions_socket_${ipAddressAndPort.port}.sock`,
    },
  };
}
