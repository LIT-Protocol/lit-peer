...
function fillMissingValues(deployFullConfig: DeployFullConfig) {
  ...
  // the default is to verify contracts
  if (deployFullConfig.deployCoreConfig?.verifyContracts === undefined) {
    deployFullConfig.deployCoreConfig.verifyContracts = true;
  }
  if (deployFullConfig.deployNodeConfig?.verifyContracts === undefined) {
    deployFullConfig.deployNodeConfig!.verifyContracts = true;
  }
}
...
export async function readDeploySensitiveConfig(): Promise<DeploySensitiveConfig> {
  ...
  const readApiKey = process.env.IPFS_API_KEY!;
  const sanitizedApiKey = readApiKey.trim().replace(/^(['"])(.*)\1$/, '$2');

  return {
    ipfsApiKey: sanitizedApiKey,
  };
}
