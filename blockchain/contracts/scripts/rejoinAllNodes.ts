...
import { StakingAdminFacet, StakingValidatorFacet } from '../typechain-types';
...
async function rejoinNodes(
  ...
  const stakingAdmin: StakingAdminFacet = await ethers.getContractAt(
    'StakingAdminFacet',
  ...
  for (const walletAddress of walletAddresses) {
    await rejoinNode(stakingAdmin, walletAddress);
  }
}

async function rejoinNode(
  stakingAdmin: StakingAdminFacet,
  walletAddress: string
) {
  console.log('rejoining', walletAddress);
  const tx = await stakingAdmin.adminRejoinValidator(realmId, walletAddress);
