...
import { StakingValidatorFacet } from '../typechain-types';
...
import {
  ...
  generateWallets,
  saveConfigFiles,
  nodeOperatorCredentialsToWalletManifestItems,
} from './utils';
const { ethers } = hre;
...
async function run() {
  ...
  const serialized = nodeOperatorCredentialsToWalletManifestItems([
    newNodeOperatorCredential,
  ]);
  ...
  await copyDirFiles(aliasNodeConfigDir, '../../rust/lit-node/lit-node/config');
}
