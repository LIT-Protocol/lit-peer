  ...
  manifest.operations.push({
    ...
    facetAddress:
      facetCutAction !== FacetCutAction.Remove
        ...
        : ethers.ZeroAddress,
    functionSelectors: functionSelectors ?? getSelectors(facetContract),
  });
...
export async function executeDiamondCutOperations(
  ...
  // Get diamond contract owner signer or use a random wallet if we're not going to actually send the txn
  const diamondOwnerSigner = diamondOwnerSignerPrivateKey
    ? new ethers.Wallet(diamondOwnerSignerPrivateKey).connect(ethers.provider)
  ...
  for (const operation of manifest.operations) {
    ...
    try {
      if (diamondOwnerSignerPrivateKey) {
        // actually send the txn
        const tx = await diamondCutFacet
          .connect(diamondOwnerSigner)
          .diamondCut([facetCut], ethers.ZeroAddress, '0x');

        console.info('Sending diamondCut txn', tx.hash);
        await tx.wait();
      } else {
        // just print the txn data, for use with a SAFE wallet or other offline signing tool
        const tx = await diamondCutFacet
          .connect(diamondOwnerSigner)
          .diamondCut.populateTransaction([facetCut], ethers.ZeroAddress, '0x');

        console.debug(
          `tx to send for facet ${operation.facetName}: ${JSON.stringify(
            tx,
            null,
            2
          )}`
