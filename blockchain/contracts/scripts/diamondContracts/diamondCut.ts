...
async function getInputsFromCliOptions(): Promise<Inputs> {
  const argv = await yargs(process.argv.slice(2)).options({
    'diamond-owner-signer-private-key': {
      type: 'string',
      describe:
        "Private key of the wallet that is the owner of the diamond contract.  This is optional and if you don't pass it, then your txn data will just be printed and the txn won't be sent.",
      required: false,
    },
  }).argv;
