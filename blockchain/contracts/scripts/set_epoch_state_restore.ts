...
async function run() {
  ...
  const stakingAdmin = await ethers.getContractAt(
    'StakingAdminFacet',
    process.env.STAKING_CONTRACT_ADDRESS as string,
    signer
  );
  ...
  const realmId = 1;
  const tx = await stakingAdmin.setEpochState(realmId, 5); // RECOVERY=5
  await tx.wait();
  console.log('Updated epoch state to Recovery');

  const state = await stakingViews.state(realmId);
